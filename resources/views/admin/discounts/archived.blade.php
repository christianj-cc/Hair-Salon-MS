...
<div class="py-3">
    <div class="flex items-center mb-6">
        ...
        <h1 class="text-2xl font-semibold text-gray-900">Archived Discounts</h1>
    </div>
    ...
    <div class="bg-white rounded-lg shadow p-3 mb-6">
        <div class="flex flex-wrap items-center gap-2">
            <div class="relative flex-1 min-w-[200px]">
                ...
                <input type="text" name="search" id="search" value="{{ request('search') }}" placeholder="Search name or code..." class="block w-full pl-10 pr-3 py-2 border border-gray-300 rounded-md leading-5 bg-white placeholder-gray-500 focus:outline-none focus:placeholder-gray-400 focus:ring-1 focus:ring-primary focus:border-primary sm:text-sm">
            </div>
            ...
            <a href="{{ route('admin.discounts.archived') }}" class="inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50">Clear</a>
        </div>
    </div>

    <form id="filter-form" method="GET" action="{{ route('admin.discounts.archived') }}" class="hidden">
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">{{ $discount->deleted_at ? $discount->deleted_at->format('M d, Y') : 'N/A' }}</td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <button @click="confirmAction('Restore Discount', 'Are you sure you want to restore this discount?', 
                        '{{ route('admin.discounts.restore', $discount->id) }}', 'POST', 'Restore', true)" class="text-green-600 hover:text-green-900" title="Restore">
                    ...
                    @empty
                    <tr>
                        <td colspan="7" class="px-6 py-4 text-center text-gray-500">No archived discounts.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
