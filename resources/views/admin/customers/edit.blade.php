...
<div class="py-3">
    <div class="flex items-center mb-6">
        <a href="{{ route('admin.customers.index', $customer) }}" class="inline-flex items-center text-gray-500 hover:text-primary transition mr-4">
    ...
    <div class="bg-white rounded-lg shadow p-6">
        <form id="editCustomerForm" method="POST" action="{{ route('admin.customers.update', $customer) }}" enctype="multipart/form-data"
            x-data="{
                  first_name: '{{ old('first_name', $customer->first_name) }}',
                  last_name: '{{ old('last_name', $customer->last_name) }}',
                  email: '{{ old('email', $customer->user->email) }}',
                  password: '',
                ...
                <div class="lg:w-2/3">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="first_name" id="first_name" x-model="first_name" value="{{ old('first_name', $customer->first_name) }}"
                        ...
                        <div>
                            ...
                            <input type="text" name="last_name" id="last_name" x-model="last_name" value="{{ old('last_name', $customer->last_name) }}"
                                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-primary focus:ring-primary" oninput="this.value = this.value.replace(/[^A-Za-z\s]/g, '')" required>
                        ...
                        <div>
                            ...
                            <input type="email" name="email" id="email" x-model="email" value="{{ old('email', $customer->user->email) }}"
                        ...
                        <div>
                            ...
                            <input type="date" name="birthdate" id="birthdate" value="{{ old('birthdate', $customer->birthdate?->format('Y-m-d')) }}"
                                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-primary focus:ring-primary">
                    ...
                    <div class="mt-6 flex gap-4">
                        <a href="{{ route('admin.customers.index') }}" class="bg-none text-gray-900 px-6 py-2 rounded-md hover:bg-primary-dark transition">Cancel</a>
                        <button type="button" @click="confirmAction('Update Customer', 'Are you sure you want to update this customer?', '#editCustomerForm', 
                        'POST', 'Update', true)" class="bg-primary text-white px-6 py-2 rounded-md hover:bg-primary-dark transition">Update Customer</button>
                    </div>
                </div>
            </div>
        </form>
