...
<div class="py-2">
    ...
    <div class="bg-white rounded-lg shadow p-3 mb-6">
        <div class="flex flex-wrap items-center gap-2">
            ...
            <div class="relative flex-1 min-w-[200px]">
                ...
                <input type="text" name="search" id="search" value="{{ request('search') }}" placeholder="Search service name..."
            ...
            <select name="category" id="category" class="block w-32 pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-primary focus:border-primary sm:text-sm rounded-md">
                ...
                <option value="Haircut" {{ request('category') == 'Haircut' ? 'selected' : '' }}>Haircut</option>
                ...
                <option value="Rebonding" {{ request('category') == 'Rebonding' ? 'selected' : '' }}>Rebonding</option>
                <option value="Manicure" {{ request('category') == 'Manicure' ? 'selected' : '' }}>Manicure</option>
                <option value="Pedicure" {{ request('category') == 'Pedicure' ? 'selected' : '' }}>Pedicure</option>
                <option value="Gel Polish" {{ request('category') == 'Gel Polish' ? 'selected' : '' }}>Gel Polish</option>
            ...
            <!-- Clear button -->
            <a href="{{ route('admin.services.index') }}" class="inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 transition focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary">
                Clear
            </a>

            <!-- Archive icon button -->
            <a href="{{ route('admin.services.archived') }}" class="ml-auto text-gray-500 hover:text-primary transition duration-150 ease-in-out transform hover:scale-110 p-2" title="Archived">
    ...
    <form id="filter-form" method="GET" action="{{ route('admin.services.index') }}" class="hidden">
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <a href="{{ route('admin.services.show', $service) }}" class="text-green-600 hover:text-green-900 mr-2" title="View Details">
                                <svg class="w-5 h-5 inline" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                                </svg>
                            </a>
                            <a href="{{ route('admin.services.edit', $service) }}" class="text-blue-600 hover:text-blue-900 mr-1" title="Edit">
                                <svg class="w-5 h-5 inline" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                </svg>
                            </a>
                            <button @click="confirmAction('Archive Service', 'Are you sure you want to archive this service?', '{{ route('admin.services.destroy', $service) }}', 'DELETE', 'Archive', true)"
